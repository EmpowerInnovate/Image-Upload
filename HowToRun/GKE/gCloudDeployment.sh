...
kubectl rollout status deployment Image-Upload-logic
...
while [ z $ImageUploadIp ]; do
    sleep 5
    kubectl get svc
    ImageUploadIp=`kubectl get service api-service --output=jsonpath='{.status.loadBalancer.ingress[0].ip}'`
	ImageUploadPort=`kubectl get service api-service --output=jsonpath='{.spec.ports[0].port}'`
done
