...
abort()
{
    echo >&2 '
***************
*** ABORTED RunImageUpload.sh ***
***************
'
    echo "An error occurred in RunImageUpload.sh . Exiting..." >&2
    exit 1
}
...
# It will switch to the ImageUpload-logic folder to run that shell file
...
echo "3/5 Deploying ImageUpload-logic deployments."
kubectl apply -f resource-manifests/deployment.yaml --record
kubectl get deployments
kubectl get pods

# It will deploy the service of sentiment-analyse-logic container
echo "4/5 Deploying service-ImageUpload-logic services."
kubectl apply -f resource-manifests/Image-Upload-logic.yaml --record
kubectl get services
kubectl get pods
